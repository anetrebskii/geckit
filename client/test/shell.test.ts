...
describe('which commands want a keyboard', () => {
  it('sends editors, pagers, passwords and logins to a terminal', () => {
    for (const command of ['vim README.md', 'sudo rm x', 'git log | less', 'ssh box', 'gh auth login', 'python3', 'EDITOR=vi git commit', 'git rebase -i HEAD~3', 'git add -p', 'git commit --amend'])
      expect(wantsKeyboard(command), command).toBe(true)
  })

  it('runs everything else here', () => {
    for (const command of ['git status', 'npm run watch', 'python3 script.py', 'git commit -m "x"', 'git commit -am x', 'git commit --amend --no-edit', 'ls -la | grep more', 'node -e 1'])
...
describe('running a command', () => {
  ...
  it('runs in the folder, keeps both streams, and says how it exited', async () => {
    const seen: string[] = []
